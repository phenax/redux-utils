...
describe('createPartialReducer', () => {
  ...
  it('should create a reducer that accepts partial state', () => {
    ...
    expect(reducer({}, { type: types.USERS.ADD.PENDING })).toEqual({ loading: true, count: 0 });
  ...
  it('should merge state and allow you to extend fallback behavior with _', () => {
    ...
    expect(reducer({ count: 0 }, { type: types.USERS.LIST.PENDING })).toEqual({ loading: true, count: 0 });
  ...
  it('should fallback for other action dispatches', () => {
    ...
    expect(reducer({}, { type: types.USERS.ADD.PENDING })).toEqual({ x: 'wow' });
